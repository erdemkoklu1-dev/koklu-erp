    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-900">Cihaz & Ürün Kataloğu</h1>
        </div>
          ...
          {([
            ...
            <button key={opt.value} onClick={() => setTab(opt.value)}
              className={`px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${
                tab === opt.value ? 'bg-[#C8102E] text-white border-[#C8102E]' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-400'
              }`}>
              {opt.label}
            </button>
          ))}
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">ÜRÜN ADI</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">KATEGORİ</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500">KDV DAHİL</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500">KDV HARİÇ</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500">PERİYODİK BAKIM</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500">DOLUM</th>
            ...
            <tbody className="divide-y">
              {loading ? (
                <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-400 text-sm">Yükleniyor...</td></tr>
              ) : goruntulenenler.length === 0 ? (
                <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-400 text-sm">Ürün bulunamadı.</td></tr>
                ...
                <tr key={u.id} className={`hover:bg-gray-50 ${!u.aktif ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    ...
                    {u.tip && <div className="text-xs text-gray-400">{u.tip}</div>}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => openEdit(u)} title="Düzenle"
                        className="text-gray-400 hover:text-gray-700 transition-colors text-base">✏️</button>
                      <button onClick={() => handleSil(u)} title="Sil"
                        className="text-gray-400 hover:text-red-600 transition-colors text-base">🗑️</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
            <div className="px-5 py-4 border-b flex items-center justify-between">
              <h2 className="text-base font-bold text-gray-900">{editId ? 'Ürünü Düzenle' : 'Yeni Ürün Ekle'}</h2>
              <button onClick={() => setModal(false)} className="text-gray-400 hover:text-gray-600 text-2xl font-bold leading-none">×</button>
            </div>
            <div className="p-5 space-y-3">
                  ...
                  {([
                    ...
                    <button key={opt.value} type="button"
                      ...
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${
                        form.kategori === opt.value ? 'bg-[#C8102E] text-white border-[#C8102E]' : 'bg-white text-gray-700 border-gray-300 hover:border-gray-500'
                      }`}>
                      {opt.label}
                    </button>
                  ))}
              ...
              {/* Ürün adı */}
              <div>
                <label className="text-xs font-medium text-gray-600 block mb-1">Ürün Adı <span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">Kapasite</label>
                  <input value={form.kapasite ?? ''} onChange={e => setForm(f => ({ ...f, kapasite: e.target.value }))}
                    placeholder="6 Kg"
                    className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C8102E]" />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">Tip</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">KDV Dahil Fiyat (₺)</label>
                  <input type="number" min={0} step="any" value={form.kdv_dahil_fiyat ?? ''}
                    onChange={e => setKdvDahil(parseFloat(e.target.value) || 0)}
                    className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C8102E]" />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">KDV Hariç Fiyat (₺)</label>
                  <input type="number" min={0} step="any" value={form.kdv_haric_fiyat ?? ''}
                    ...
                    className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C8102E] bg-gray-50" />
                </div>
              </div>
              ...
              {form.kategori === 'dolum' && (
                <div className="grid grid-cols-2 gap-3 pt-1 border-t">
                  <div>
                    <label className="text-xs font-medium text-gray-600 block mb-1">Periyodik Bakım (₺)</label>
                  ...
                  <div>
                    <label className="text-xs font-medium text-gray-600 block mb-1">Dolum Fiyatı (₺)</label>
      ...
      {topluModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            <div className="px-5 py-4 border-b flex items-center justify-between">
              <h2 className="text-base font-bold text-gray-900">Toplu Fiyat Güncelle</h2>
              <button onClick={() => setTopluModal(false)} className="text-gray-400 hover:text-gray-600 text-2xl font-bold leading-none">×</button>
            </div>
            <div className="p-5 space-y-4">
                ...
                <div className="flex gap-1 flex-wrap">
                  {([
                    ...
                    <button key={opt.value} type="button"
                      ...
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${
                        topluFiltre === opt.value ? 'bg-gray-800 text-white border-gray-800' : 'bg-white text-gray-700 border-gray-300 hover:border-gray-500'
                      }`}>
                      {opt.label}
                    </button>
                  ))}
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 block mb-1">İşlem</label>
                <div className="flex gap-1">
                  <button type="button" onClick={() => setTopluYon('artis')}
                    className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${topluYon === 'artis' ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-700 border-gray-300 hover:border-gray-500'}`}>
                    📈 Zam (%)
                  </button>
                  <button type="button" onClick={() => setTopluYon('azalis')}
                    className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${topluYon === 'azalis' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-gray-700 border-gray-300 hover:border-gray-500'}`}>
                    📉 İndirim (%)
                  </button>
                </div>
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600 block mb-1">Oran (%)</label>
