    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <Link href="/fiyat-teklifleri" className="text-gray-500 text-sm hover:text-gray-700">← Fiyat Teklifleri</Link>
          <span className="text-gray-300">/</span>
          <h1 className="text-lg font-bold text-gray-900 font-mono">{teklif.teklif_no}</h1>
        </div>
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-4">
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="text-xl font-bold text-gray-900">{musteriAdi}</span>
                {teklif.musteri_sehir && (
                  <span className="text-sm text-gray-500">— {teklif.musteri_sehir}</span>
                )}
              </div>
            ...
            <div className="text-right">
              ...
              <div className="text-xs text-gray-500">{teklif.para_birimi} · KDV {
        ...
        <div className="bg-white border rounded-lg overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500">S.NO</th>
                <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500">AÇIKLAMA</th>
                <th className="text-right px-4 py-2 text-xs font-semibold text-gray-500">ADET</th>
                <th className="text-right px-4 py-2 text-xs font-semibold text-gray-500">BİRİM FİYAT</th>
                <th className="text-right px-4 py-2 text-xs font-semibold text-gray-500">TOPLAM</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {(kalemler ?? []).map((k) => (
                <tr key={k.id}>
                  <td className="px-4 py-3 text-sm text-gray-500">{k.sira_no}</td>
                  <td className="px-4 py-3 text-sm text-gray-900">{k.aciklama}</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-700">{k.miktar}</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-700">{fmt(k.birim_fiyat, currency)}</td>
                  <td className="px-4 py-3 text-sm text-right font-medium text-gray-900">{fmt(k.toplam, currency)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot className="border-t bg-gray-50">
              <tr>
                <td colSpan={4} className="px-4 py-2 text-sm text-right text-gray-600">KDV Hariç Toplam</td>
                <td className="px-4 py-2 text-sm text-right font-medium">{fmt(teklif.ara_toplam ?? 0, currency)}</td>
              </tr>
              {teklif.kdv_durumu !== 'yok' && (
                <tr>
                  <td colSpan={4} className="px-4 py-2 text-sm text-right text-gray-600">KDV (%{teklif.kdv_orani})</td>
                  <td className="px-4 py-2 text-sm text-right font-medium">{fmt(teklif.kdv_tutari ?? 0, currency)}</td>
                </tr>
              )}
              <tr>
                <td colSpan={4} className="px-4 py-2 text-right font-bold text-gray-900">Genel Toplam</td>
        ...
        {teklif.notlar && (
          <div className="bg-white border rounded-lg p-4">
            <h2 className="text-sm font-semibold text-gray-700 mb-2">Notlar</h2>
            <p className="text-sm text-gray-700 whitespace-pre-wrap">{teklif.notlar}</p>
          </div>
        )}
        ...
        {teklif.ticari_sartname_ekli && teklif.ticari_sartname_metni && (
          <div className="bg-white border rounded-lg p-4">
            <h2 className="text-sm font-semibold text-gray-700 mb-2">Ticari Şartname</h2>
            <pre className="text-xs text-gray-600 whitespace-pre-wrap font-sans leading-relaxed">{teklif.ticari_sartname_metni}</pre>
          </div>
        )}
