...
const DURUM_OPTIONS: { value: Durum; label: string; className: string }[] = [
  { value: 'taslak',     label: 'Taslak',     className: 'border-gray-300 text-gray-700' },
  ...
  { value: 'iptal',      label: 'İptal',      className: 'border-gray-400 text-gray-500' },
]
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b px-6 py-4 flex items-center gap-3">
        ...
        <Link href={`/fiyat-teklifleri/${id}`} className="text-gray-500 text-sm hover:text-gray-700">← {teklif_no}</Link>
        <span className="text-gray-300">/</span>
        <h1 className="text-lg font-bold text-gray-900">Durum Güncelle</h1>
      </div>
      ...
      <div className="p-6 max-w-md mx-auto">
        <div className="bg-white border rounded-lg p-5 space-y-4">
          ...
          <div>
            <div className="text-xs font-medium text-gray-600 mb-2">Yeni Durum Seçin</div>
            <div className="space-y-2">
              {DURUM_OPTIONS.map(opt => (
                <label key={opt.value}
                  className={`flex items-center gap-3 p-3 border-2 rounded-lg cursor-pointer transition-colors ${
                    secilenDurum === opt.value
                      ? 'bg-gray-50 ' + opt.className
