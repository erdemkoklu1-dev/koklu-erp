    ...
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-800 border rounded-xl p-4">
          <div className="text-xs text-gray-500 dark:text-gray-400 mb-0.5">Toplam Tedarikçi Borcu</div>
        ...
        <div className="bg-white dark:bg-gray-800 border rounded-xl p-4">
          <div className="text-xs text-gray-500 dark:text-gray-400 mb-0.5">Toplam Tedarikçi</div>
          <div className="text-xl font-bold text-gray-900 dark:text-gray-100">{suppliers.length}</div>
          <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{(manuelTedarikciler ?? []).length} manuel · {suppliers.filter(s => s.invoiceCount > 0).length} faturalı</div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {suppliers.length === 0 ? (
          ...
          return (
            <div key={sup.name} className="bg-white dark:bg-gray-800 border rounded-xl overflow-hidden">
              <div className={`px-5 py-3 border-b flex items-center justify-between ${sup.overdueCount > 0 ? 'bg-red-50' : 'bg-gray-50 dark:bg-gray-700'}`}>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {sup.manuelData && sup.invoiceCount > 0 && (
                    <span className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border px-2 py-0.5 rounded-full">
                      Kayıtlı
                    </span>
                  )}
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                      ...
                      {(sup.invoices ?? []).map(inv => {
                        ...
                        return (
                          <tr key={inv.id} className={`${isOverdue ? 'bg-red-50/50' : ''} hover:bg-gray-50`}>
                            ...
                            <td className="px-4 py-2.5 text-sm text-gray-800 dark:text-gray-200 font-medium text-right">
                    ...
                    <tfoot className="border-t bg-gray-50 dark:bg-gray-700/50">
                      <tr>
                        <td colSpan={3} className="px-4 py-2 text-xs font-semibold text-gray-600 dark:text-gray-300">Toplam</td>
                        <td className="px-4 py-2 text-sm font-bold text-gray-800 dark:text-gray-200 text-right">{formatCurrency(sup.totalInvoice)}</td>
                        <td className="px-4 py-2 text-sm font-bold text-green-700 text-right">{formatCurrency(sup.totalPaid)}</td>
                        <td className={`px-4 py-2 text-sm font-bold text-right ${kalan > 0 ? 'text-orange-600' : 'text-gray-400 dark:text-gray-500'}`}>
