  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-gray-900">Gecikmiş Faturalar</h2>
          <p className="text-xs text-gray-400 mt-0.5">Vadesi geçmiş ve tam ödenmeyen tüm gelen faturalar</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white border rounded-xl p-4">
          <div className="text-xs text-gray-500 mb-0.5">Tedarikçi Sayısı</div>
          <div className="text-xl font-bold text-gray-900">{supMap.size}</div>
          <div className="text-xs text-gray-400 mt-0.5">gecikmiş borç olan</div>
        </div>
        <div className="bg-white border rounded-xl p-4">
          <div className="text-xs text-gray-500 mb-0.5">En Uzun Gecikme</div>
          <div className="text-xl font-bold text-gray-900">{enUzunGecikme} gün</div>
          <div className="text-xs text-gray-400 mt-0.5">en eski vadeli fatura</div>
        </div>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="space-y-4">
          {groups.map(group => (
            <div key={group.name} className="bg-white border border-red-200 rounded-xl overflow-hidden">
                ...
                <table className="w-full text-sm">
                  <thead className="border-b bg-gray-50">
                    <tr>
                      ...
                      <th className="text-left px-4 py-2 text-xs font-medium text-gray-500">Fatura Tarihi</th>
                      <th className="text-left px-4 py-2 text-xs font-medium text-gray-500">Vade Tarihi</th>
                      ...
                      <th className="text-right px-4 py-2 text-xs font-medium text-gray-500">Gecikme</th>
                    ...
                    {group.invoices.map(inv => {
                      ...
                      return (
                        <tr key={inv.id} className="hover:bg-red-50/30">
                          ...
                          <td className="px-4 py-2.5 text-gray-600 text-xs">{formatTRDate(inv.invoice_date)}</td>
                          <td className="px-4 py-2.5 text-red-600 font-medium text-xs">
                            {formatTRDate(due)}
                            {!inv.due_date && <span className="ml-1 text-gray-400">(fatura tarihi)</span>}
                          </td>
                          <td className="px-4 py-2.5 text-right text-gray-800 font-medium">{formatCurrency(inv.total_amount)}</td>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Gecikme</th>
                ...
                {rows.map(inv => {
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-red-50/30">
                      ...
                      <td className="px-4 py-3 text-gray-600 text-sm">{formatTRDate(inv.invoice_date)}</td>
                      <td className="px-4 py-3">
                        <span className="text-red-600 font-medium text-sm">{formatTRDate(due)}</span>
                        {!inv.due_date && (
                          <span className="ml-1 text-xs text-gray-400">(fatura tarihi)</span>
                        )}
                      </td>
