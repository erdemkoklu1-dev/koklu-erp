  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-gray-900">Sabit Giderler</h2>
          <p className="text-xs text-gray-400 mt-0.5">Abonelik ve periyodik ödemelerinizi buradan yönetin</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white border rounded-xl p-4">
          <div className="text-xs text-gray-500">Aktif Gider</div>
          <div className="text-2xl font-bold text-gray-900">{expenses?.length ?? 0}</div>
        </div>
        ...
        <div className="bg-white border rounded-xl p-4">
          <div className="text-xs text-gray-500">Yıllık Toplam Maliyet</div>
          <div className="text-2xl font-bold text-gray-900">{formatCurrency(monthlyTotal * 12)}</div>
        </div>
      </div>
        ...
        {(expenses ?? []).length === 0 ? (
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Gider Adı</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Kategori</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Periyot</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Her Ayın</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Tutar</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">KDV</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Aylık Karş.</th>
              ...
              {(expenses ?? []).map(e => {
                ...
                return (
                  <tr key={e.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">{e.day_of_month}. günü</td>
                    <td className="px-4 py-3 text-sm font-medium text-gray-900 text-right">{formatCurrency(e.amount)}</td>
            ...
            <tfoot className="border-t bg-gray-50">
              <tr>
                <td colSpan={6} className="px-4 py-3 text-sm font-semibold text-gray-700">Aylık Toplam</td>
