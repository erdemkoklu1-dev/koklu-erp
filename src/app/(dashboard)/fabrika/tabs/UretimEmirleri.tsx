...
const DURUM_BADGE: Record<string, string> = {
  'planlandı':   'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300',
      ...
      <div className="bg-white dark:bg-gray-800 border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400 dark:text-gray-500">Kayıt yok</td></tr>
            )}
            {filtered.map(e => (
              <tr key={e.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{e.planlanan_baslangic ?? '—'}</td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-lg shadow-2xl">
            <div className="px-6 py-4 border-b flex items-center justify-between">
              <h3 className="font-semibold text-gray-900 dark:text-gray-100">Yeni Üretim Emri</h3>
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm">
                  ...
                  <div className="mt-2 flex gap-2">
                    ...
                    <button onClick={() => { setUyariOnay(false); setStokUyari([]) }}
                      className="px-3 py-1.5 text-xs border rounded-lg text-gray-600 dark:text-gray-300">İptal</button>
                  </div>
                </div>
              )}
              ...
              {!uyariOnay && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Planlanan Başlangıç</label>
                    <input type="date" value={form.planlanan_baslangic}
                      onChange={e => setForm(f => ({ ...f, planlanan_baslangic: e.target.value }))}
                      className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C8102E]" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Planlanan Bitiş</label>
                    <input type="date" value={form.planlanan_bitis}
                      onChange={e => setForm(f => ({ ...f, planlanan_bitis: e.target.value }))}
                      className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C8102E]" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Sorumlu</label>
