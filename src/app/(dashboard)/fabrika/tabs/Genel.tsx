...
export default function Genel() {
  ...
  const durumBadge = (durum: string) => {
    if (durum === 'planlandı') return <span className="px-2 py-0.5 text-xs rounded-full bg-gray-100 text-gray-600">Planlandı</span>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border p-4">
          <div className="text-xs text-gray-500 mb-1">Toplam Stok Değeri</div>
          <div className="text-2xl font-bold text-gray-900">
            ₺{(kpi?.stokDegeri ?? 0).toLocaleString('tr-TR', { minimumFractionDigits: 2 })}
          </div>
        </div>
        <div className={`rounded-xl border p-4 ${kpi && kpi.kritikStokSayisi > 0 ? 'bg-red-50 border-red-200' : 'bg-white'}`}>
          <div className="text-xs text-gray-500 mb-1">Kritik Stok</div>
          <div className={`text-2xl font-bold ${kpi && kpi.kritikStokSayisi > 0 ? 'text-red-600' : 'text-gray-900'}`}>
            {kpi?.kritikStokSayisi ?? 0}
            {kpi && kpi.kritikStokSayisi > 0 && <span className="text-sm ml-1">hammadde</span>}
          </div>
        </div>
        <div className="bg-white rounded-xl border p-4">
          <div className="text-xs text-gray-500 mb-1">Aktif Üretim Emirleri</div>
          <div className="text-2xl font-bold text-gray-900">{kpi?.aktifEmir ?? 0}</div>
        </div>
        <div className="bg-white rounded-xl border p-4">
          <div className="text-xs text-gray-500 mb-1">Bu Ay Tamamlanan</div>
          <div className="text-2xl font-bold text-gray-900">{kpi?.buAyTamamlanan ?? 0}</div>
        </div>
      </div>
      ...
      {kritik.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4">
          ...
          <div className="space-y-2">
            {kritik.map(h => (
              <div key={h.id} className="flex items-center justify-between bg-white rounded-lg px-4 py-2 border border-red-100">
      ...
      <div className="bg-white border rounded-xl overflow-hidden">
        ...
        {aktifEmirleri.length === 0 ? (
          <div className="px-5 py-8 text-center text-sm text-gray-400">Aktif üretim emri yok</div>
        ) : (
