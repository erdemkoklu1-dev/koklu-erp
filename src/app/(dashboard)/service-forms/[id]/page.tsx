    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-700">
      <div className="bg-white dark:bg-gray-800 border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100">{form.form_number}</h1>
          {form.control_number && (
        ...
        <div className="flex gap-2">
          <Link href={`/service-forms/${id}/edit`}
            className="border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors">
            ✏️ Düzenle
          </Link>
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-5">
        <div className="bg-white dark:bg-gray-800 border rounded-lg p-5">
          <div className="grid grid-cols-3 gap-4">
            <div>
              <div className="text-xs text-gray-400 dark:text-gray-500 mb-1">Müşteri</div>
              <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">{customer?.full_name}</div>
              {customer?.phone && <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{customer.phone}</div>}
              {customer?.address && <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{customer.address}</div>}
            </div>
            <div>
              <div className="text-xs text-gray-400 dark:text-gray-500 mb-1">Teknisyen</div>
              <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{form.technician_name ?? '-'}</div>
              <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">Servis Tarihi</div>
              <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{form.service_date}</div>
            </div>
            <div>
              <div className="text-xs text-gray-400 dark:text-gray-500 mb-1">Form No</div>
              <div className="text-sm font-mono font-bold text-[#C8102E]">{form.form_number}</div>
              <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">Servis Türü</div>
        ...
        <div className="grid grid-cols-4 gap-3">
          <div className="bg-white dark:bg-gray-800 border rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{totalDevices}</div>
          ...
          <div className="bg-white dark:bg-gray-800 border rounded-lg p-4 text-center">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">✅ Tümü Sağlam</div>
          </div>
          <div className="bg-white dark:bg-gray-800 border rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-red-600">{hasarli}</div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">⚠️ Hasarlı Satır</div>
          </div>
          <div className="bg-white dark:bg-gray-800 border rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{items?.length ?? 0}</div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">📋 Cihaz Tipi</div>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-gray-700 border-b">
                  <tr>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400">CİHAZ</th>
                    <th className="text-center px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400">ADET</th>
                    <th className="text-center px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400">GÖVDE</th>
                    <th className="text-center px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400">VANA</th>
                    <th className="text-center px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400">HORTUM</th>
                    <th className="text-center px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400">SAAT</th>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400">NOT</th>
                  </tr>
                </thead>
                  ...
                  {items.map((item: any) => {
                    ...
                    return (
                      <tr key={item.id} className={`hover:bg-gray-50 ${!allOk ? 'bg-red-50' : ''}`}>
                        <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-100">{item.device_name}</td>
                        <td className="px-4 py-3 text-sm text-center font-bold text-gray-700 dark:text-gray-300">{item.quantity}</td>
        ...
        {(form.general_notes || form.customer_note || form.next_service_date) && (
          <div className="bg-white dark:bg-gray-800 border rounded-lg p-5 space-y-3">
            {form.general_notes && (
              <div>
                <div className="text-xs text-gray-400 dark:text-gray-500 mb-1">Genel Notlar</div>
                <div className="text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 rounded-lg p-3">{form.general_notes}</div>
              </div>
            )}
            {form.customer_note && (
              <div>
                <div className="text-xs text-gray-400 dark:text-gray-500 mb-1">Müşteriye Not</div>
                <div className="text-sm text-gray-700 dark:text-gray-300 bg-blue-50 border border-blue-100 rounded-lg p-3">{form.customer_note}</div>
              </div>
            )}
            {form.next_service_date && (
              <div>
                <div className="text-xs text-gray-400 dark:text-gray-500 mb-1">Sonraki Servis Tarihi</div>
