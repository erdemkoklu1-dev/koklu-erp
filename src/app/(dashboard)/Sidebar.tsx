...
export default function Sidebar({ permissionSet }: { permissionSet: SidebarPermissionSet }) {
  const pathname = usePathname()

  function canSee(item: NavItem): boolean {
    if (!item.modul) return true
    if (permissionSet.isAdmin) return true
    if (item.href === '/yonetim' && permissionSet.isBackupManager) return true
    if (item.adminOnly) return false
    return moduleKeysWithAliases(item.modul).some(key => permissionSet.modulIzinleri[key] === true)
  }
