  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link href={`/personel/${personel.id}`} className="text-gray-400 dark:text-gray-500 hover:text-gray-600"><ChevronLeft size={20} /></Link>
        <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Personel Düzenle</h1>
      </div>
      ...
      <div className="flex items-center justify-end gap-3">
        <Link href={`/personel/${personel.id}`} className="px-4 py-2 border rounded-lg text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50">İptal</Link>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3 border-b pb-2">{title}</p>
