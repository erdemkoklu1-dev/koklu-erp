...
const thCls = 'px-3 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide'
  ...
  return (
    <div className="p-6 max-w-screen-xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/personel" className="text-gray-400 dark:text-gray-500 hover:text-gray-600"><ChevronLeft size={20} /></Link>
        <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Maaş Bordrosu</h1>
      </div>

      {/* Kontroller */}
      <div className="bg-white dark:bg-gray-800 rounded-xl border p-4 flex flex-wrap items-center gap-4">
        <div>
          <label className="text-xs text-gray-500 dark:text-gray-400 block mb-1">Dönem</label>
        ...
        <div className="flex-1 flex items-center justify-end gap-3">
          ...
          <button onClick={handlePrint} className="flex items-center gap-2 border px-4 py-2 rounded-lg text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Toplam Brüt', val: toplamBrut, cls: 'text-gray-800 dark:text-gray-200' },
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-gray-400 dark:text-gray-500 text-sm">Yükleniyor...</div>
        ) : (
                  ...
                  <tr key={p.id} className="hover:bg-gray-50">
                    <td className={tdCls}>
                      <div>
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500">{p.pozisyon ?? ''} {p.subeler ? `· ${p.subeler.ad}` : ''}</p>
                      </div>
                    </td>
                    ...
                    <td className={tdCls + ' text-right text-gray-500 dark:text-gray-400'}>{formatCurrency(brut + sgkIsveren)}</td>
                    <td className={tdCls}>
                      {odeme
                        ...
                        : <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">Bekleniyor</span>
                      }
                    </td>
                    <td className={tdCls}>
                      {!odeme && brut > 0 && (
                      ...
                      {odeme && odeme.odendi && (
                        <button onClick={() => toggleOdendi(odeme.id, true)} className="text-xs text-gray-400 dark:text-gray-500 hover:underline">Geri Al</button>
                      )}
                    </td>
                  </tr>
                ))}
                {satirlar.length === 0 && (
                  <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-400 dark:text-gray-500">Aktif personel bulunamadı.</td></tr>
                )}
              </tbody>
              <tfoot className="border-t bg-gray-50 dark:bg-gray-700">
                <tr>
                  <td className={tdCls + ' font-bold text-gray-800 dark:text-gray-200'}>Toplam ({satirlar.length} personel)</td>
