    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b px-6 py-4 flex items-center justify-between no-print">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-900">{sube.ad}</h1>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setRaporAcik(true)} className="text-sm border text-gray-600 px-4 py-2 rounded-lg hover:bg-gray-50">
            Rapor Al
          </button>
      ...
      <div className="p-6 space-y-5">
        {/* Şube bilgi kartı */}
        <div className="bg-white border rounded-xl p-5 no-print">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            {sube.adres      && <div><span className="text-gray-400 text-xs">Adres</span><div className="text-gray-800 mt-0.5">{sube.adres}{sube.ilce ? `, ${sube.ilce}` : ''}{sube.sehir ? ` / ${sube.sehir}` : ''}</div></div>}
            {sube.telefon    && <div><span className="text-gray-400 text-xs">Telefon</span><div className="text-gray-800 mt-0.5">{sube.telefon}</div></div>}
            {sube.email      && <div><span className="text-gray-400 text-xs">E-posta</span><div className="text-gray-800 mt-0.5">{sube.email}</div></div>}
            {sube.yetkili_kisi && <div><span className="text-gray-400 text-xs">Yetkili</span><div className="text-gray-800 mt-0.5">{sube.yetkili_kisi} {sube.yetkili_telefon ? `(${sube.yetkili_telefon})` : ''}</div></div>}
            {sube.acilis_tarihi && <div><span className="text-gray-400 text-xs">Açılış</span><div className="text-gray-800 mt-0.5">{formatTRDate(sube.acilis_tarihi)}</div></div>}
            {sube.notlar     && <div className="col-span-2"><span className="text-gray-400 text-xs">Notlar</span><div className="text-gray-800 mt-0.5">{sube.notlar}</div></div>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 no-print">
          {[
            { label: 'Müşteri', value: musteriler.length, cls: 'text-gray-900', fmt: false },
            ...
            { label: 'Bu Ay Servis', value: buAyServis, cls: 'text-gray-900', fmt: false },
        ...
        <div className="no-print">
          <div className="flex gap-1 border-b bg-white rounded-t-xl px-4 overflow-x-auto">
            {TABS.map(t => (
              <button key={t.key} onClick={() => setActiveTab(t.key)}
                className={`px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${activeTab === t.key ? 'border-[#C8102E] text-[#C8102E]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          <div className="bg-white border border-t-0 rounded-b-xl overflow-hidden">
            ...
            {activeTab === 'musteriler' && (
              <div>
                <div className="flex items-center justify-between px-4 py-3 border-b bg-gray-50">
                  <span className="text-sm font-semibold text-gray-700">{musteriler.length} Müşteri</span>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y">
                    ...
                    {musteriler.length === 0 && <tr><td colSpan={4} className="px-4 py-10 text-center text-gray-400 text-sm">Bu şubeye atanmış müşteri yok.</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
            ...
            {activeTab === 'giden' && (
              <div>
                <div className="flex gap-6 px-4 py-3 border-b bg-gray-50 text-sm">
                  <span className="text-gray-500">Toplam: <span className="font-semibold text-gray-900">{formatCurrency(gidenFaturalar.reduce((s, f) => s + (f.total_amount ?? 0), 0))}</span></span>
                  <span className="text-gray-500">Tahsil: <span className="font-semibold text-green-600">{formatCurrency(gidenFaturalar.reduce((s, f) => s + (f.paid_amount ?? 0), 0))}</span></span>
                  <span className="text-gray-500">Kalan: <span className="font-semibold text-orange-600">{formatCurrency(toplamAlacak)}</span></span>
                </div>
                <table className="w-full">
                  ...
                  <tbody className="divide-y">
                    ...
                    {gidenFaturalar.length === 0 && <tr><td colSpan={6} className="px-4 py-10 text-center text-gray-400 text-sm">Bu şubeye ait giden fatura yok.</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
            ...
            {activeTab === 'gelen' && (
              <div>
                <div className="flex gap-6 px-4 py-3 border-b bg-gray-50 text-sm">
                  <span className="text-gray-500">Toplam Borç: <span className="font-semibold text-red-600">{formatCurrency(toplamBorc)}</span></span>
                  <span className="text-gray-500">Toplam: <span className="font-semibold text-gray-900">{formatCurrency(gelenFaturalar.reduce((s, f) => s + (f.total_amount ?? 0), 0))}</span></span>
                </div>
                <table className="w-full">
                  <thead className="bg-gray-50 border-b"><tr>
                    <th className={thCls}>Fatura No</th>
                  ...
                  <tbody className="divide-y">
                    ...
                    {gelenFaturalar.length === 0 && <tr><td colSpan={6} className="px-4 py-10 text-center text-gray-400 text-sm">Bu şubeye ait gelen fatura yok.</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
            ...
            {activeTab === 'servis' && (
              <div>
                <div className="px-4 py-3 border-b bg-gray-50 text-sm text-gray-500">
                  Toplam {servisFormlari.length} form · Bu ay: <span className="font-semibold text-gray-900">{buAyServis}</span>
                </div>
                <table className="w-full">
                  <thead className="bg-gray-50 border-b"><tr>
                  ...
                  <tbody className="divide-y">
                    ...
                    {servisFormlari.length === 0 && <tr><td colSpan={5} className="px-4 py-10 text-center text-gray-400 text-sm">Bu şubeye ait servis formu yok.</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
                  ...
                  <div className="flex gap-1">
                    {(['gelir', 'gider'] as const).map(t => (
                      <button key={t} onClick={() => setGelirGiderAlt(t)}
                        className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${gelirGiderAlt === t ? 'bg-[#C8102E] text-white' : 'text-gray-500 hover:bg-gray-100'}`}>
                    ...
                    <div>
                      <table className="w-full">
                        ...
                        <tbody className="divide-y">
                          ...
                          {rows.length === 0 && <tr><td colSpan={5} className="px-4 py-10 text-center text-gray-400 text-sm">Kayıt yok.</td></tr>}
                        </tbody>
                      </table>
                      {rows.length > 0 && (
                        <div className="px-4 py-3 border-t bg-gray-50 flex justify-end text-sm">
                          <span className="text-gray-500">Dönem Toplamı: <span className={`font-bold ${gelirGiderAlt === 'gelir' ? 'text-green-600' : 'text-red-600'}`}>{formatCurrency(toplam)}</span></span>
                        </div>
                      )}
                    </div>
                  )
                ...
                <div className="space-y-2">
                  {maliOzet.slice(-6).map(m => (
                    <div key={m.label} className="flex items-center gap-3 text-xs">
                      <span className="w-20 text-gray-500 text-right shrink-0">{m.label.split(' ')[0]}</span>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm border rounded-xl overflow-hidden">
                    ...
                    <tfoot className="border-t bg-gray-50">
                      <tr>
                        <td className={tdCls + ' font-bold text-gray-900'}>Toplam (12 Ay)</td>
              ...
              <div className="p-4 space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-sm font-semibold text-gray-700">Personel ({personeller.length})</p>
                  ...
                  <table className="w-full border rounded-xl overflow-hidden text-sm">
                    ...
                    <tbody className="divide-y">
                      {personeller.map(p => {
                        const DURUM_CLS: Record<string,string> = { aktif:'bg-green-100 text-green-700', izinli:'bg-blue-100 text-blue-700', deneme:'bg-yellow-100 text-yellow-700', istifa:'bg-red-100 text-red-700', cikis:'bg-gray-100 text-gray-500' }
                        ...
                        return (
                          <tr key={p.id} className="hover:bg-gray-50">
                            ...
                            <td className={tdCls}><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${DURUM_CLS[p.durum] ?? 'bg-gray-100 text-gray-500'}`}>{DURUM_LABEL[p.durum] ?? p.durum}</span></td>
                          </tr>
                        )
                      })}
                      {personeller.length === 0 && <tr><td colSpan={4} className="px-4 py-6 text-center text-gray-400 text-sm">Bu şubede kayıtlı personel yok.</td></tr>}
                    </tbody>
                  </table>
                </div>
                ...
                {calisanlar.length > 0 && (
                  <div>
                    <p className="text-sm font-semibold text-gray-700 mb-3">Sistem Kullanıcıları ({calisanlar.length})</p>
                    <table className="w-full border rounded-xl overflow-hidden text-sm">
                      <thead className="bg-gray-50 border-b"><tr>
                        <th className={thCls}>Ad Soyad</th>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b">
              <h3 className="font-bold text-gray-900">Müşteri Ata</h3>
              <button onClick={() => { setMusteriAtaAcik(false); setSeciliMusteriler(new Set()) }} className="text-gray-400 hover:text-gray-600 text-xl">×</button>
            </div>
            ...
            <div className="overflow-y-auto flex-1">
              {ataListesi.map(m => (
                <div key={m.id} onClick={() => setSeciliMusteriler(prev => { const n = new Set(prev); n.has(m.id) ? n.delete(m.id) : n.add(m.id); return n })}
                  ...
                  <div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 ${seciliMusteriler.has(m.id) ? 'bg-[#C8102E] border-[#C8102E]' : 'border-gray-300'}`}>
              ...
              {ataListesi.length === 0 && <div className="px-5 py-10 text-center text-gray-400 text-sm">Atanacak müşteri bulunamadı.</div>}
            </div>
            <div className="px-5 py-4 border-t flex gap-3">
              ...
              <button onClick={() => { setMusteriAtaAcik(false); setSeciliMusteriler(new Set()) }} className="px-4 py-2 border rounded-lg text-sm text-gray-600">İptal</button>
            </div>
          </div>
        </div>
      )}
      ...
      {ggAcik && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-5 py-4 border-b">
              <h3 className="font-bold text-gray-900">Gelir / Gider Ekle</h3>
              <button onClick={() => setGgAcik(false)} className="text-gray-400 hover:text-gray-600 text-xl">×</button>
            </div>
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-medium text-gray-700">Tip</label>
                  <select value={ggForm.tip} onChange={e => setGgForm(p => ({ ...p, tip: e.target.value }))} className={inp + ' bg-white'}>
            ...
            <div className="flex gap-3 px-5 pb-5">
              ...
              <button onClick={() => setGgAcik(false)} className="px-4 py-2 border rounded-lg text-sm text-gray-600">İptal</button>
            </div>
          </div>
        </div>
      )}
      ...
      {raporAcik && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm">
            <div className="flex items-center justify-between px-5 py-4 border-b">
              <h3 className="font-bold text-gray-900">Rapor Al</h3>
              <button onClick={() => setRaporAcik(false)} className="text-gray-400 hover:text-gray-600 text-xl">×</button>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="text-sm font-medium text-gray-700">Dönem Yılı</label>
                <select value={raporDonemYil} onChange={e => setRaporDonemYil(e.target.value)} className={inp + ' bg-white'}>
                  {[0, 1, 2].map(i => { const y = new Date().getFullYear() - i; return <option key={y} value={y}>{y}</option> })}
                </select>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">Ay (boş = tüm yıl)</label>
                <select value={raporDonemAy} onChange={e => setRaporDonemAy(e.target.value)} className={inp + ' bg-white'}>
              ...
              <div>
                ...
                <select value={raporTip} onChange={e => setRaporTip(e.target.value as any)} className={inp + ' bg-white'}>
            ...
            <div className="flex gap-3 px-5 pb-5">
              ...
              <button onClick={() => setRaporAcik(false)} className="px-4 py-2 border rounded-lg text-sm text-gray-600">İptal</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="print-only hidden p-8 text-sm">
        ...
        <div className="flex items-start justify-between mb-6 pb-4 border-b-2 border-gray-800">
          <div>
            ...
            <div className="text-gray-600 mt-0.5">Dönem: {printDonem}</div>
          </div>
        ...
        {(printTip === 'tam' || printTip === 'mali') && (
          <div className="mb-6">
            <table className="w-full border-collapse text-xs mb-4">
              <tbody>
                <tr><td className="py-0.5 pr-4 text-gray-500 w-32">Tip</td><td className="py-0.5 font-medium">{cfg.label}</td><td className="py-0.5 pr-4 text-gray-500 w-32">Şehir</td><td className="py-0.5 font-medium">{sube.sehir} {sube.ilce && `/ ${sube.ilce}`}</td></tr>
                {sube.adres && <tr><td className="py-0.5 text-gray-500">Adres</td><td className="py-0.5 font-medium" colSpan={3}>{sube.adres}</td></tr>}
                {sube.yetkili_kisi && <tr><td className="py-0.5 text-gray-500">Yetkili</td><td className="py-0.5 font-medium">{sube.yetkili_kisi}</td><td className="py-0.5 text-gray-500">Yetkili Tel</td><td className="py-0.5 font-medium">{sube.yetkili_telefon ?? '-'}</td></tr>}
              </tbody>
            </table>
            {/* KPI */}
