    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-700">
      <div className="bg-white dark:bg-gray-800 border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100">Şubeler</h1>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {(subeler ?? []).map((sube: any) => {
            ...
            return (
              <div key={sube.id} className={`bg-white dark:bg-gray-800 border rounded-xl p-5 flex flex-col gap-3 ${!sube.aktif ? 'opacity-60' : ''}`}>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h3 className="text-base font-bold text-gray-900 dark:text-gray-100">{sube.ad}</h3>
                    <div className="text-sm text-gray-500 dark:text-gray-400">{sube.sehir}{sube.ilce ? `, ${sube.ilce}` : ''}</div>
                  </div>
                </div>

                {sube.telefon && <div className="text-sm text-gray-600 dark:text-gray-300">📞 {sube.telefon}</div>}
                {sube.yetkili_kisi && <div className="text-sm text-gray-600 dark:text-gray-300">👤 {sube.yetkili_kisi}</div>}

                <div className="grid grid-cols-3 gap-2 pt-2 border-t">
                  <div className="text-center">
                    <div className="text-xs text-gray-400 dark:text-gray-500">Müşteri</div>
                    <div className="text-sm font-bold text-gray-700 dark:text-gray-300">{musteriMap.get(sube.id) ?? 0}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xs text-gray-400 dark:text-gray-500">Alacak</div>
                ...
                <div className="flex gap-2">
                  <Link href={`/subeler/${sube.id}`} className="flex-1 text-center text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-3 py-1.5 rounded-lg hover:bg-gray-200">
                    Detay →
                  </Link>
                  <Link href={`/subeler/${sube.id}/edit`} className="text-xs border text-gray-600 dark:text-gray-300 px-3 py-1.5 rounded-lg hover:bg-gray-50">
                    Düzenle
                  </Link>
                </div>
              </div>
          ...
          {(!subeler || subeler.length === 0) && (
            <div className="col-span-3 text-center py-16 text-gray-400 dark:text-gray-500 text-sm">
        ...
        {(subeler ?? []).length > 0 && (
          <div className="bg-white dark:bg-gray-800 border rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b bg-gray-50 dark:bg-gray-700">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Şube Karşılaştırması</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-700 border-b">
                  <tr>
                    <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Şube</th>
                    <th className="text-right px-4 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Müşteri</th>
                    ...
                    <th className="text-right px-4 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Net</th>
                    <th className="text-right px-4 py-2.5 text-xs font-semibold text-gray-500 dark:text-gray-400">Bu Ay Servis</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={sube.id} className="hover:bg-gray-50">
                        <td className="px-4 py-2.5">
                          <Link href={`/subeler/${sube.id}`} className="font-medium text-gray-900 dark:text-gray-100 hover:text-[#C8102E] hover:underline">{sube.ad}</Link>
                          <span className="text-xs text-gray-400 dark:text-gray-500 ml-2">{sube.sehir}</span>
                        </td>
                        <td className="px-4 py-2.5 text-right text-gray-700 dark:text-gray-300">{musteriMap.get(sube.id) ?? 0}</td>
                        ...
                        <td className="px-4 py-2.5 text-right text-gray-700 dark:text-gray-300">{stats.servis}</td>
                      </tr>
                    )
                  })}
                </tbody>
