      ...
      <div className="p-6 max-w-5xl mx-auto space-y-5">
        ...
        <div className="bg-white border rounded-lg p-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <div className="text-xs text-gray-400 mb-1">Yetkili</div>
              <div className="text-sm font-medium">{customer.authorized_person ?? '-'}</div>
            </div>
            <div>
              <div className="text-xs text-gray-400 mb-1">Yetkili Telefonu</div>
        ...
        <div className="grid grid-cols-4 gap-3">
          <div className="bg-white border rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-gray-900">{devices?.reduce((s, d) => s + (d.quantity ?? 1), 0) ?? 0}</div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500">Ürün Adı</th>
                  <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500">Adet</th>
              ...
              <tbody className="divide-y">
                {devices && devices.length > 0 ? devices.map((device) => (
                  <tr key={device.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-sm text-gray-600">{device.quantity ?? 1}</td>
                    <td className="px-4 py-2.5 text-sm text-gray-600">{device.brand ?? '-'}</td>
                    <td className="px-4 py-2.5 text-sm text-gray-600">{formatTRDate(device.last_fill_date)}</td>
                    <td className="px-4 py-2.5 text-sm text-gray-600">{formatTRDate(device.control1_date)}</td>
                    <td className="px-4 py-2.5 text-sm text-gray-600">{formatTRDate(device.control2_date)}</td>
                    <td className="px-4 py-2.5 text-sm text-gray-600">{formatTRDate(device.control3_date)}</td>
                    <td className="px-4 py-2.5 text-sm text-gray-600">{formatTRDate(device.expiry_date)}</td>
        ...
        <div className="bg-white border rounded-lg overflow-hidden">
          <div className="px-5 py-3 border-b flex items-center justify-between bg-gray-50">
            <div className="flex items-center gap-3">
              <h2 className="text-sm font-semibold text-gray-900">Ön Kayıtlar</h2>
              {bekleyenTutar > 0 && (
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    ...
                    <th className="text-right px-4 py-2 text-xs font-semibold text-gray-500">Tutar</th>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500">Durum</th>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500">Fatura</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {(onKayitlar ?? []).map(k => {
                    ...
                    return (
                      <tr key={k.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-2.5 text-sm text-gray-900 max-w-xs truncate">{k.aciklama}</td>
                        <td className="px-4 py-2.5 text-sm text-gray-600 text-right">{k.miktar} {k.birim}</td>
