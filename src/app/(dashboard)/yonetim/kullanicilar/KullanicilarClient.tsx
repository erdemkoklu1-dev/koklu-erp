...
function RolBadge({ rol }: { rol: Rol | null }) {
  if (!rol) return <span className="text-xs text-gray-400">—</span>
  return (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-gray-800">Kullanıcılar ({kullanicilar.length})</h2>
      ...
      <div className="bg-white border rounded-lg overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b">
            <tr>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">AD SOYAD</th>
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">SON GİRİŞ</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">DURUM</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">İŞLEMLER</th>
            </tr>
          </thead>
          <tbody className="divide-y">
            {yukleniyor ? (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-400">Yükleniyor...</td></tr>
            ) : kullanicilar.length === 0 ? (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-400">Kullanıcı bulunamadı.</td></tr>
            ) : kullanicilar.map(k => (
              <tr key={k.id} className={`hover:bg-gray-50 ${!k.aktif ? 'opacity-50' : ''}`}>
                <td className="px-4 py-3 text-sm font-medium text-gray-900">{k.ad_soyad || '—'}</td>
                <td className="px-4 py-3 text-sm text-gray-600">{k.email}</td>
                <td className="px-4 py-3"><RolBadge rol={k.roller} /></td>
                <td className="px-4 py-3 text-sm text-gray-600">{k.departman || '—'}</td>
                <td className="px-4 py-3 text-xs text-gray-500">
                  {k.son_giris ? formatTRDate(k.son_giris.slice(0, 10)) : '—'}
                </td>
                <td className="px-4 py-3">
                  <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium border ${k.aktif ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                    {k.aktif ? 'Aktif' : 'Pasif'}
                  </span>
                </td>
                <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            <div className="px-5 py-4 border-b flex items-center justify-between">
              <h3 className="text-base font-bold text-gray-900">Yeni Kullanıcı</h3>
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              <div>
                <label className="text-xs font-medium text-gray-600">Geçici Şifre *</label>
              ...
              <div>
                <label className="text-xs font-medium text-gray-600">Departman</label>
              ...
              <div className="flex items-center gap-3">
                ...
                <div onClick={() => setFormAktif(p => !p)}
                  ...
                  <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${formAktif ? 'left-5' : 'left-0.5'}`} />
                </div>
              </div>
            </div>
            <div className="px-5 py-4 border-t flex gap-3">
              ...
              <button onClick={() => setModal(null)} className="flex-1 border py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50">İptal</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal === 'duzenle' && secili && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            <div className="px-5 py-4 border-b flex items-center justify-between">
              <h3 className="text-base font-bold text-gray-900">Kullanıcı Düzenle</h3>
              <button onClick={() => setModal(null)} className="text-gray-400 hover:text-gray-600 text-xl">×</button>
            </div>
            <div className="px-5 py-4 space-y-3">
              {hata && <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded px-3 py-2">{hata}</div>}
              ...
              <div className="text-sm text-gray-700">
                <span className="font-medium">{secili.ad_soyad}</span> — <span className="text-gray-500">{secili.email}</span>
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600">Rol</label>
                <select value={formRol} onChange={e => setFormRol(e.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C8102E]">
      ...
      {modal === 'sifre' && secili && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            <div className="px-5 py-4 border-b flex items-center justify-between">
              <h3 className="text-base font-bold text-gray-900">Şifre Sıfırlama Linki</h3>
              <button onClick={() => setModal(null)} className="text-gray-400 hover:text-gray-600 text-xl">×</button>
            </div>
            <div className="px-5 py-4 space-y-3">
              <p className="text-sm text-gray-600"><span className="font-medium">{secili.email}</span> için şifre sıfırlama linki:</p>
              {!resetLink ? (
                <div className="text-sm text-gray-400">Oluşturuluyor...</div>
              ) : (
                <div className="bg-gray-50 border rounded-lg p-3 break-all text-xs font-mono text-gray-700">{resetLink}</div>
              )}
              {resetLink && (
            ...
            <div className="px-5 py-4 border-t">
              <button onClick={() => setModal(null)} className="w-full py-2 border rounded-lg text-sm text-gray-600 hover:bg-gray-50">Kapat</button>
            </div>
          </div>
        </div>
      )}
