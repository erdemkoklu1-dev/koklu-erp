    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white border rounded-lg p-4">
          <div className="text-xs text-gray-500">Bugün Aktif Kullanıcı</div>
          <div className="text-2xl font-bold text-gray-900 mt-1">{bugunAktif}</div>
        </div>
      ...
      {/* Filtreler */}
      <form onSubmit={filtrele} className="bg-white border rounded-lg p-4 flex flex-wrap items-center gap-3">
      ...
      <div className="bg-white border rounded-lg overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b">
            <tr>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">KULLANICI</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">İŞLEM</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">ZAMAN</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">IP</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">TARAYICI</th>
            </tr>
          </thead>
          <tbody className="divide-y">
            {kayitlar.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-gray-400">Kayıt bulunamadı.</td></tr>
            ) : kayitlar.map(k => {
              const conf = ISLEM_CONFIG[k.islem_tipi]
              return (
                <tr key={k.id} className={`hover:bg-gray-50 ${k.islem_tipi === 'basarisiz_giris' ? 'bg-red-50' : ''}`}>
                  <td className="px-4 py-3">
                    <div className="text-sm font-medium text-gray-900">{k.ad_soyad || '—'}</div>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600">{fmt(k.created_at)}</td>
                  <td className="px-4 py-3 text-xs text-gray-500 font-mono">{k.ip_adresi || '—'}</td>
                  <td className="px-4 py-3 text-xs text-gray-400 max-w-xs truncate" title={k.tarayici ?? ''}>
